...
tickets AS (
    SELECT
        ...
        MODE() WITHIN GROUP (ORDER BY ticket_type)                          AS most_common_ticket_type,
        MODE() WITHIN GROUP (ORDER BY channel)                              AS preferred_support_channel
...
SELECT
    ...
    CASE
        WHEN p.last_purchase_date < CURRENT_DATE - INTERVAL '90 days'      THEN TRUE
